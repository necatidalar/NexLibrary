...
public partial class FrmMemberDetail : Form
{
    ...
    public FrmMemberDetail(MemberApiService memberApiService, int memberId)
    {
        InitializeComponent();
    ...
    private async void FrmMemberDetail_Load(object sender, EventArgs e)
    {
        await LoadMemberDetailAsync();
    }

    private async Task LoadMemberDetailAsync()
    {
        try
        {
            var detail = await _memberApiService.GetByIdAsync(_memberId);
        ...
        {
            MessageBox.Show(
                $"Üye detayı yüklenirken hata oluştu:\n{ex.Message}",
                "NexLibrary",
                MessageBoxButtons.OK,
                MessageBoxIcon.Error);
        }
    ...
    private void BindDetail(MemberDetailResponse detail)
    {
        lblTitle.Text = detail.UyeAdiSoyadi;
        ...
        if (detail.DinamikAlanlar.Count == 0)
        {
            ...
            {
                Text = "Bu üye için dinamik alan değeri bulunmuyor.",
